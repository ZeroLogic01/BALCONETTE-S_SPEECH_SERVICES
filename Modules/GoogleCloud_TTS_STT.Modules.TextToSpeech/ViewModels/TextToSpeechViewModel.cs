...
namespace GoogleCloud_TTS_STT.Modules.TextToSpeech.ViewModels
{
    internal class TextToSpeechViewModel : BindableBase
    {
        ...
        public string SsmlToolTip { set; get; } = "Avoid Special Characters &, \", <, & > in text. These characters are reserved for SSML tags. Use Special Characters context menu instead.";
        ...
        public DelegateCommand<string> SpecialCharactersCommand { get; set; }
        ...
        public TextToSpeechViewModel(IEventAggregator eventAggregator)
        {
            ...
            SsmlBreakCommand = new DelegateCommand(ExecuteSsmlBreak, CanExecuteSsmlBreak);
            SpecialCharactersCommand = new DelegateCommand<string>(ExecuteSpecialCharactersCommand, CanExecuteSpecialCharactersCommand);
        ...
        private bool CanExecuteSsmlBreak()
        {
            return !IsTextOptionSelected;
        }

        private async void ExecuteSsmlExphasize(string level)
        {
            //string selectedText = SelectedText.Replace("&", "&amp;")
            //    .Replace("\"", "&quot;")
            //    .Replace("<", "&lt;")
            //    .Replace(">", "&gt;");
        ...
        {
            return !string.IsNullOrWhiteSpace(_selectedText) && !IsTextOptionSelected;
        }

        private void ExecuteSpecialCharactersCommand(string text)
        {
            _eventAggregator.GetEvent<SpecialCharacterEvent>().Publish(text);
        }
        private bool CanExecuteSpecialCharactersCommand(string arg)
        {
            return !IsTextOptionSelected;
        }
